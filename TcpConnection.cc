...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    if(!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        if (nwrote >= 0)
        {
            ...
            {
                //既然在这里数据全部发送完成，就不用再给channel设置epollout事件了,epollout对应channel里面的writeCallback_
                loop_->queueInLoop(
...
void TcpConnection::handleError()
{
    ...
    LOG_ERROR("TcpConnection::handleError name:%s - S0_ERROR:%d \n", name_.c_str(), err);
}
